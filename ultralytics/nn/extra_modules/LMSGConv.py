...
class BLMSGConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, kernels=[1, 3, 5], weights=[0.2, 0.3, 0.5]):
        super().__init__()
        ...
        min_ch = (min_ch // self.groups) * self.groups
        ...
        # 固定权重
        # assert len(weights) == len(kernels), "weights must match the number of kernels"
        # self.weights = weights

        # 初始化可学习权重
        self.weights = nn.Parameter(torch.ones(self.groups))
    ...
    #     x1 = self.cv1(x)
    #     # 使用每个卷积核的输出并根据权重调整比例
    #     outputs = [conv(x1) * weight for conv, weight in zip(self.convs, self.weights)]
    ...
    def forward(self, x):
        x1 = self.cv1(x)
        outputs = [conv(x1) for conv in self.convs]

        # 对权重进行 softmax 归一化
        weights = torch.softmax(self.weights, dim=0)
        x2 = torch.cat([output * weight for output, weight in zip(outputs, weights)], dim=1)
        x = self.cv2(x2)
        return x
