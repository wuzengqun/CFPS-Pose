...
from ..modules.conv import Conv, DWConv, DSConv, RepConv, GhostConv, autopad, ConvBNAct
...
'GSConv', 'GSConvns', 'VoVGSCSP', 'VoVGSCSPns', 'VoVGSCSPC', 'C3k2_CloAtt', 'SCConv', 'C3k2_SCConv', 'ScConv', 'C3k2_ScConv',
           ...
           'C3k2_MutilScaleEdgeInformationEnhance', 'C3k2_FFCM'
           ]
...
class CSPStage(nn.Module):
    def __init__(self,
        ...
        for i in range(n):
            ...
            next_ch_in = ch_mid
        # self.conv3 = Conv(ch_mid * len(self.convs) + ch_first, ch_out, 1)
...
class SP_Conv(nn.Module):
    def __init__(self, in_channels, out_channels, k=1, p=0, g=1, act=True):
        super(SP_Conv, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=k, padding=p, groups=g)
        self.bn = nn.BatchNorm2d(out_channels)
...
class ContextGuideFusionModule(nn.Module):
    def __init__(self, inc) -> None:
        ...
        # 使用 ECA 替代 SE_Block
        self.channel_attention = ECA(inc[1] * 2, k_size=3)

        # 使用轻量级空间注意力模块
        self.spatial_attention = SP_Conv(inc[1] * 2, inc[1] * 2, k=3, p=1, g=inc[1] * 2, act=False)
    ...
    def forward(self, x):
        ...
        # 使用 ECA 获取通道注意力
        x_concat = self.channel_attention(x_concat)

        # 使用轻量级空间注意力模块
        spatial_weights = self.spatial_sigmoid(self.spatial_attention(x_concat))
        x_concat = x_concat + spatial_weights

        # 分割特征图，分别获取 x0 和 x1 的权重
