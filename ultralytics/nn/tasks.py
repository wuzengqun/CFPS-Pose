...
C3K2_CLASS = (C3k2_Faster, C3k2_ODConv, C3k2_Faster_EMA, C3k2_DBB, C3k2_DeepDBB, C3k2_WDBB, C3k2_CloAtt, C3k2_SCConv, C3k2_ScConv, C3k2_EMSC, C3k2_EMSCP, C3k2_KW, C3k2_DySnakeConv, C3k2_DCNv2, C3k2_DCNv3,
              ...
              C3k2_FFCM, C3k2_MutilScaleEdgeInformationEnhance
              )
...
class BaseModel(nn.Module):
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            ...
            for m in self.model.modules():
                if isinstance(m, RepConv):
                    m.switch_to_deploy()  # 调用 switch_to_deploy 进行融合
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            LDConv, CSP_MSCB, CSP_PMSFA, RFAConv, RFCBAMConv, RFCAConv, BLMSGConv, GSConv, GGConv, MSGConv, MSGAConv, DualConv, My_dualConv,
        ) + C3K2_CLASS):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in ((
                ...
                CSPStage, SPDConv, RepBlock, CSP_EDLAN, RGCSPELAN, CSP_PTB, XSSBlock, CSP_MSCB, CSP_PMSFA
            ) + C3K2_CLASS):
